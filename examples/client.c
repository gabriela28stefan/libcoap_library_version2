...
//FIXME decomment if you want to use block options
//#define FLAGS_BLOCK 0x01
#define FLAGS_BLOCK 0x00
...
//static str output_file = {11 , "file_output" };   /* output file name */
...
static void parse_data(data_t* sensor_data, const unsigned char* buf){

	unsigned char* tmp = strdup(buf);
	unsigned char* token;
	...
	token = strtok(NULL, ",");
	sensor_data->temp = strtol(token, (char **)NULL, 10);
